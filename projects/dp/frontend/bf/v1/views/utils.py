...
def user_subscribed(user):	
	...
	if not user.subscription_set.filter(end_date__gte=datetime.datetime.now()).filter(active=True).all():
		return False

	return True


def user_logged_in_and_subscribed(user):
...
#DEFAULT_ALPHABET = 'abcdefghijklmnopqrstuvwxzy123456789'
DEFAULT_ALPHABET = 'r4nm6txq5fd1wbz7s3la8c2eyigkvuophj9'
